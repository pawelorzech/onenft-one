...
ONE: coins backed by USDC on Base, one series of 25,000 at a time, 50 Master Coins and 100 founder coins per series, four backing classes (5, 10, 25, 50 USDC). Fifth collection of onenft.click (hub repo `~/Programowanie/onenft-hub`), built 2026-09-06 in the skeleton of the sisters (`~/Programowanie/onenft-faces` is the closest). Live at https://one.onenft.click since 2026-09-06, contract `0xF597D7bD4467A501a7634dD53Be63E1c7261bcdB` on Base mainnet (see `docs/DEPLOYMENTS.md`); the audit trail is in `docs/DECISIONS.md`. Operational identifiers go in `CLAUDE.local.md` (gitignored).
...
- The browser does the writing: the mint box on `/` connects, switches the wallet to the right chain, reads the USDC balance and allowance, approves the exact total when it must, sends `mint(class, count, wallet)` with `vrfFeeWei()` as the transaction value (one Chainlink fee per transaction, whatever the count), reads the new ids from the `Minted` logs and polls `/api/coin/<id>` until each coin opens, keeping the transaction in `localStorage` per chain, contract and wallet so a refresh picks it up. The author's wallet also sees `mintFounder`, with the band spelled out from `founderWindow(series)`: founder coin k is minted inside coins `pace*(k-1)+1` to `pace*k` of the series or forfeited, one coin per band, and the button is shut outside it. Claim and redeem sit on `/coin/<id>` and on a wallet's page, shown only to the wallet that owns the coin; Burn is disabled until `redeemableAt`, and while a coin is sealed until its `SEALED_ESCAPE` date, when the contract opens the door for a coin the VRF never answered for; the date is in the button either way, and Claim is open the whole time. Calldata is built from selectors in `src/contract.ts`; there is no ABI encoder in the page. Every revert the contract can throw at a minter or a holder has a written sentence in `REVERTS`, matched by selector, so a wallet never shows the reader four bytes of hex.
