...
- The browser does the writing: the mint box on `/` connects, switches the wallet to the right chain, reads the USDC balance and allowance, approves the exact total when it must, sends `mint(class, count, wallet)` with `vrfFeeWei()` as the transaction value (one Chainlink fee per transaction, whatever the count), reads the new ids from the `Minted` logs and polls `/api/coin/<id>` until each coin opens, keeping the transaction in `localStorage` per chain, contract and wallet so a refresh picks it up. The author's wallet also sees `mintFounder`, with the pacing spelled out: a founder coin per `FOUNDER_PACE` coins of the series, the count capped at what is open and the button shut until then. Claim and redeem sit on `/coin/<id>` and on a wallet's page, shown only to the wallet that owns the coin; Burn is disabled while a coin is sealed and until `redeemableAt`, with the date in the button, while Claim is open the whole time. Calldata is built from selectors in `src/contract.ts`; there is no ABI encoder in the page. Every revert the contract can throw at a minter or a holder has a written sentence in `REVERTS`, matched by selector, so a wallet never shows the reader four bytes of hex.
