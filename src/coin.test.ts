...
describe("coin", () => {
  ...
  test("the svg is a 64 by 64 pixel grid with crisp edges and only palette colours", () => {
  ...
  test("fifty masters, each named, each its own bytes, twenty-five modes", () => {
  ...
  test("legends read on every ground", () => {
    for (let i = 0n; i < 300n; i++) {
      ...
      let slot = 0;
      for (let x = 0; x < N && !slot; x++) slot = c.grid.get(x, 3);
      expect(slot).not.toBe(0);
      expect(c.colors[slot]).not.toBe(c.colors[0]);
    }
  });
});
