...
import { DEFAULT_BACKINGS } from "./contract.ts";

/** A sealed coin, the image the site shows while nothing is minted. */
export function placeholderCoin(): Coin {
  return renderCoin({ seed: 0n, number: 1, series: 1, backing: DEFAULT_BACKINGS[1], yieldBps: 0, master: -1, founder: false, sealed: true });
}
...
export function coinOfSeed(seed: bigint, yieldBps = 0, master = -1): Coin {
  return renderCoin({ seed, number: 0, series: 1, backing: DEFAULT_BACKINGS[1], yieldBps, master, founder: false });
}
